QT =
CONFIG += exceptions c++14  # required config
CONFIG -= qt
...
precompile_header {
	SOURCES += $$CRYPTOPP_SRC_DIR/pch.cpp
...
cryptopp_disable_asm {
	message("disabeling assembler")
	QT_CRYPTOPP_DEFINES += CRYPTOPP_DISABLE_ASM
} else:cryptopp_disable_mixed_asm {
	message("disabling mixed assembler")
	QT_CRYPTOPP_DEFINES += CRYPTOPP_DISABLE_MIXED_ASM
}

cryptopp_no_os_dependence {
	message("disabeling os dependence")
